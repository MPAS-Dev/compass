...
``landice/thwaites/fo_decomposition_test`` and
``landice/thwaites/fo-depthInt_decomposition_test``.  They run short (2-day)
integrations of the model forward in time on 16 (``16proc_run`` step) and
then on 32 cores (``32proc_run`` step) to make sure the resulting prognostic
variables agree within the tolerances used by the test.

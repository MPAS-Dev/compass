...
from compass.landice.tests.ismip7_run.ismip7_ais.set_up_experiment import (
    SetUpExperiment,
)
from compass.testcase import TestCase

# Define the full experiment matrix per the ISMIP7 protocol
EXPERIMENTS = {
    'historical_CESM2-WACCM': {
        'scenario': 'historical', 'model': 'CESM2-WACCM',
        'start_time': '1850-01-01_00:00:00',
...
class Ismip7Ais(TestCase):
    """
    A test case for automated setup of a suite of standardized
    ISMIP7 simulations for the Antarctic Ice Sheet.
    """
    ...
    def configure(self):
        """
        Set up the desired ISMIP7 AIS experiments.

        Read the experiment list from config and add a SetUpExperiment
        step for each.
        """
        config = self.config
        exp_list_str = config.get('ismip7_run_ais', 'exp_list')
        ...
        # each experiment (step) should be submitted manually
        self.steps_to_run = []

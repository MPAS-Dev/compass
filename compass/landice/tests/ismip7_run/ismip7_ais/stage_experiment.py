...
def symlink_experiment_inputs(step, exp, exp_info):  # noqa: C901
    """
    Symlink all the input files (initial condition, forcing, melt
    parameters, region mask, reference surface, and, for
    non-historical experiments, the restart file from the
    corresponding historical run) needed to run an ISMIP7 AIS
    experiment into a step's work directory.

    Parameters
    ----------
    step : compass.Step
        The step whose work directory the inputs should be staged into
    ...
    -------
    filenames : dict
        A dictionary of the file names (basenames) that were symlinked
        (and the forcing intervals to use), to be used when rendering
        the streams file template
    """
    config = step.config
    ...
    if scenario == 'ctrl':
        ...
        os.symlink(ctrl_tf_path,
                   os.path.join(step.work_dir, tf_fname))
        ...
        if len(smb_files) == 1:
            ...
            os.symlink(smb_files[0],
                       os.path.join(step.work_dir, smb_fname))
        else:
        ...
        if len(temp_files) == 1:
            ...
            os.symlink(temp_files[0],
                       os.path.join(step.work_dir, temp_fname))
        else:
        ...
        if len(runoff_files) == 1:
            ...
            os.symlink(runoff_files[0],
                       os.path.join(step.work_dir, runoff_fname))
        else:
        ...
        if len(smb_grad_files) == 1:
            ...
            os.symlink(smb_grad_files[0],
                       os.path.join(step.work_dir, smb_grad_fname))
        ...
        if len(temp_grad_files) == 1:
            ...
            os.symlink(temp_grad_files[0],
                       os.path.join(step.work_dir, temp_grad_fname))
    ...
    if not is_historical:
        ...
        os.symlink(f"../{hist_exp}/rst.2015-01-01.nc",
                   os.path.join(step.work_dir, 'rst.2015-01-01.nc'))
        with open(os.path.join(step.work_dir, "restart_timestamp"),
                  "w") as text_file:
            text_file.write("2015-01-01_00:00:00")

    return {
        'init_cond_path': init_cond_path,
        'init_cond_fname': init_cond_fname,
        'melt_params_fname': melt_params_fname,
        'region_mask_fname': region_mask_fname,
        'reference_surface_fname': reference_surface_fname,
        'smb_fname': smb_fname,
        'temp_fname': temp_fname,
        'tf_fname': tf_fname,
        'runoff_fname': runoff_fname,
        'smb_grad_fname': smb_grad_fname,
        'temp_grad_fname': temp_grad_fname,
        'forcing_interval_monthly': forcing_interval_monthly,
        'forcing_interval_annual': forcing_interval_annual,
    }


def configure_experiment_namelist_and_streams(step, exp_info, filenames,
                                              out_name='landice'):
    """
    Add the namelist and streams files needed to run an ISMIP7 AIS
    experiment to a step.

    Parameters
    ----------
    step : compass.Step
        The step to add the namelist and streams files to
    ...
    filenames : dict
        The dictionary returned by :py:func:`symlink_experiment_inputs`

    out_name : str, optional
        The suffix to use for the namelist and streams file names, i.e.
        ``namelist.<out_name>`` and ``streams.<out_name>``
    """
    config = step.config
    section = config['ismip7_run_ais']
    ...
    namelist_out_name = f'namelist.{out_name}'
    streams_out_name = f'streams.{out_name}'

    # --- Set up streams ---
    stream_replacements = {
        'input_file_init_cond': filenames['init_cond_fname'] if is_historical
        else 'USE_RESTART_FILE_INSTEAD',
        'input_file_region_mask': filenames['region_mask_fname']
        if is_historical else 'USE_RESTART_FILE_INSTEAD',
        'input_file_melt_params': filenames['melt_params_fname'],
        'input_file_reference_surface': filenames['reference_surface_fname'],
        'input_file_SMB_forcing': filenames['smb_fname'],
        'input_file_temperature_forcing': filenames['temp_fname'],
        'input_file_TF_forcing': filenames['tf_fname'],
        'input_file_runoff_forcing': filenames['runoff_fname'],
        'input_file_smb_gradient_forcing': filenames['smb_grad_fname'],
        'input_file_temperature_gradient_forcing':
            filenames['temp_grad_fname'],
        'forcing_interval_monthly': filenames['forcing_interval_monthly'],
        'forcing_interval_annual': filenames['forcing_interval_annual'],
    }

    step.add_streams_file(
        RESOURCE_LOCATION,
        'streams.landice.template',
        out_name=streams_out_name,
        template_replacements=stream_replacements)

    # --- Set up namelist ---
    step.add_namelist_file(
        RESOURCE_LOCATION, 'namelist.landice',
        out_name=namelist_out_name)

    # PIO options
    pio_stride = section.getint('pio_stride')
    io_tasks = step.ntasks // pio_stride
    options = {'config_pio_stride': f'{pio_stride}',
               'config_pio_num_iotasks': f'{io_tasks}'}
    step.add_namelist_options(options=options,
                              out_name=namelist_out_name)
    ...
    if sea_level_model:
        ...
        file_slm_nl = os.path.join(step.work_dir, 'namelist.sealevel')
        with open(file_slm_nl, 'w') as handle:
            handle.write(text)

        os.makedirs(os.path.join(step.work_dir, 'OUTPUT_SLM/'),
                    exist_ok=True)
        os.makedirs(os.path.join(step.work_dir, 'ICELOAD_SLM/'),
                    exist_ok=True)
        ...
        for map_file in ('mapfile_mali_to_slm.nc',
            ...
            os.symlink(os.path.join(map_dir, map_file),
                       os.path.join(step.work_dir, map_file))

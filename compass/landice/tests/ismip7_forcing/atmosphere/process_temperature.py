...
class ProcessTemperature(Step):
    """
    A step for processing ISMIP7 near-surface air temperature (tas) data.
    ...
    def run(self):
        ...
        input_path = os.path.join(base_path_ismip7, "tas", "v2")
        file_pattern = f"tas_AIS_{model}_{scenario}_SDBN1-2000m_v2_*.nc"
    ...
    def _combine_and_rename(self, remapped_files, output_file):
        ...
        # Rename variable
        if "tas" in ds:
            ds = ds.rename({"tas": "surfaceAirTemperature"})

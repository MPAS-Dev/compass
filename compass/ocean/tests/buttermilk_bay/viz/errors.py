...
interp_dem = RegularGridInterpolator((ds_dem.x.values, ds_dem.y.values), values.T, method='nearest')
...
#points = np.array([[2.8, 0.53], [1.9, 1.66], [2.4, 3.029], [2.51, 3.027], [1.26, 1.56]])
points = np.array([[2.8, 0.53], [1.9, 1.66], [3.0, 3.25], [2.6, 3.25], [1.26, 1.56]])
# No #3
...
for j,res in enumerate(resolutions):
    ...
    interp_standard = NearestNDInterpolator(xy, np.squeeze(ds_standard.layerThickness.values).T)
...
log_wct_standard_interp = np.interp(np.log10(rmse_subgrid), np.log10(np.flip(rmse_standard)), np.log10(np.flip(wct_standard)))
...
for i in range(3):
    #ax.plot([wct_subgrid[i], wct_standard_interp[i]],[rmse_subgrid[i], rmse_subgrid[i]],'k-')
    ax.annotate("",xy=(wct_subgrid[i], rmse_subgrid[i]), xytext=(wct_standard_interp[i], rmse_subgrid[i]), arrowprops=dict(arrowstyle="->"))
    x = 0.5*(wct_subgrid[i] + wct_standard_interp[i])
    ax.text(0.95*x, 1.03*rmse_subgrid[i], f'{round(speedup[i],2)}'+r'$\times$', horizontalalignment='center')

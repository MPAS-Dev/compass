...
def plot_subgrid(ax, X, Y, bathy, solution, mesh, shape, ssh, points):
    ...
    for iCell in range(mesh.dims['nCells']):
        ...
        inwet = np.where(inpt & ((bathy+h0) < ssh[tsnap,iCell]) & (ssh[tsnap,iCell] < 2.0 ))
    ...
    c = ax.contourf(X,Y,solution.reshape(shape), vmin=sol_min, vmax=sol_max, levels=levels, cmap=cmap, extend='both')
...
print(levels)
c = ax.contourf(X,Y,bathy,cmap=cmocean.cm.topo, vmin=vmin, vmax=vmax, levels=levels, extend='both')
tick_step = 5
...
#resolutions = [256]
#points = np.array([[2.8, 0.53], [1.9, 1.66], [2.4, 3.029], [2.51, 3.027], [1.26, 1.56]])*1000
points = np.array([[2.8, 0.53], [1.9, 1.66], [3.0, 3.25], [2.6, 3.25], [1.26, 1.56]])*1000
...
for i,res in enumerate(resolutions):
    ...
    cm = plot_subgrid(ax[i], X, Y, bathy.ravel(), solution.ravel(), ds_mesh, shape, ssh, points)

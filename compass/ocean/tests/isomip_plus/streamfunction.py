...
class Streamfunction(Step):
    """
    A step for computing the barotropic and overturning streamfunctions from
    ...
    def __init__(self, test_case, resolution, experiment):
        ...
        experiment : {'Ocean0', 'Ocean1', 'Ocean2'}
            The ISOMIP+ experiment
        """
        super().__init__(test_case=test_case, name='streamfunction')
        ...
        self.add_output_file('barotropicStreamfunction.nc')
        self.add_output_file('overturningStreamfunction.nc')
    ...
    def run(self):
        ...
        dx = self.config.getfloat('isomip_plus_streamfunction', 'osf_dx')
        dz = self.config.getfloat('isomip_plus_streamfunction', 'osf_dz')
        ...
        _compute_barotropic_streamfunction(dsMesh, ds, out_dir,
                                           show_progress=show_progress)

        _compute_overturning_streamfunction(dsMesh, ds, out_dir, dx=dx, dz=dz,
                                            show_progress=show_progress)
...
def _compute_overturning_streamfunction(dsMesh, ds, out_dir, dx=2e3, dz=5.,
                                        show_progress=True):
    """
    ...
    osfFileName = '{}/overturningStreamfunction.nc'.format(out_dir)
...
def _compute_horizontal_transport_mpas(ds, dsMesh, outFileName):
    ...
    minLevelEdgeBot = minLevelCell[cell0]
    mask = numpy.logical_or(cell0 == -1,
                            minLevelCell[cell1] > minLevelEdgeBot)
    minLevelEdgeBot[mask] = minLevelCell[cell1][mask]
    ...
    layerThicknessEdge = layerThicknessEdge.where(mask, other=0.)

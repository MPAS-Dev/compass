...
def make_convergence_arrays(tcdata):
  """
    Collects data from a set of test case runs at different resolutions to use for
    convergence data analysis and plotting.

    Arguments:
      tcdata: a dictionary whose keys are the resolution values for a sphere_transport test case
...
def plot_convergence(ax, tcname, dlambda, resvals, linf1, l21, linf2, l22, linf3, l23):
  """
    ...
    Arguments:
      ...
      resvals: An integer array of resolution values, e.g., [120, 240] for QU120 and QU240
...
def plot_filament(ax, tcname, resvals, filament):
  ...
  """
#   prop_cycle = plt.rcParams['axes.prop_cycle']
#   color_cycle = cycler("color", prop_cycle.by_key()['color'][:6])
#   line_cycle = cycler('ls',['-', '--', '-.', ':'])
#   marker_cycle = cycler('marker', ['o', 's', '^', '+', 'd', '*'])
#   mcycle = (color_cycle + marker_cycle) * line_cycle
  tau = np.linspace(0,1,21)
#   for i, (r, sty) in enumerate(zip(resvals, mcycle)):
#     ax.plot(tau, filament[i], label="QU{}".format(r), **sty)
  for i, r in enumerate(resvals):
...
def plot_over_and_undershoot_errors(ax, tcname, resvals, u1, o1, u2, o2, u3, o3):
  if len(u1[0]) == 13:
    ...
    ax.set(title=tcname, xlabel='time (day)', ylabel='rel. over/undershoot')
#     ax.legend(mode=expand, ncols=6, bbox_to_anchor=(0,0), loc='lower left')

...
def init_triplot_axes(ax):
  lw = 0.4
  topline = Line2D([0.1, 1.0], [0.9, 0.9], color='k', linestyle='-', linewidth=lw)
  botline = Line2D([0.1, 1.0], [0.9, 0.1], color='k', linestyle='-', linewidth=lw)
  rightline = Line2D([1,1], [0.1,0.9], color='k', linestyle='-',linewidth=lw)
...
class Analysis(Step):
    ...
    def run(self):
        ...
        for i, r in enumerate(rvals):
          ...
          ax.plot(ds.variables["tracer2"][6,:,1], ds.variables["tracer3"][6,:,1], 'r.',
            markersize=1)
        fig.savefig("correlatedTracers2D_tripplots.pdf")

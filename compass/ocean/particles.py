...
def build_particle_simple(f_grid, f_name, f_decomp, buoySurf, vertexReconst=1,
                          horizTreatment=1, timeInt=2, verticalTreatment=4):
    """
    ...
    f_out.variables['xParticle'][0, :] = \
        np.tile(f_grid.variables['xCell'][:], nBuoySurf)
    f_out.variables['yParticle'][0, :] = \
        np.tile(f_grid.variables['yCell'][:], nBuoySurf)
    f_out.variables['zParticle'][0, :] = \
        np.tile(f_grid.variables['zCell'][:], nBuoySurf)
    f_out.variables['buoyancyParticle'][0, :] = \
        (np.tile(buoySurf, (nparticles, 1))).reshape(
            nparticles*nBuoySurf, order='F').copy()
